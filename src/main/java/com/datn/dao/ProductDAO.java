...
// "(SELECT MAX(p2.id) FROM products p2 GROUP BY p2.product_Category_Id)",
// nativeQuery = true)
// List<Product> findLatestProductsPerCategory();

// @Query("SELECT p FROM Product p WHERE p.productCategory.name LIKE :name order
// by p.id desc")
...
// // @Query("SELECT p FROM Product p JOIN OrderDetail od ON p.id =
// od.product.id
// // WHERE p.productCategory.id = :productCategoryId GROUP BY p.id ORDER BY
// // SUM(od.quantity) DESC")
// // Page<Product>
// findBestSellingProductsByCategory(@Param("productCategoryId")
