...
public class ProductServiceImpl implements ProductService {
    ...
    public Product create(Product entity, MultipartFile imageFile) {
        ...
        if (!imageFile.isEmpty()) {
            entity.setImage_url(
                    param.save(imageFile, "D:\\Vs_Java5\\Assignment\\src\\main\\resources\\static\\images").getName());
        }
        ...
        if (dao.existsById(entity.getId())) {
            if (!imageFile.isEmpty()) {
                entity.setImage_url(param
                        .save(imageFile, "D:\\Vs_Java5\\Assignment\\src\\main\\resources\\static\\images").getName());
            } else {
    ...
    @Override
    public Page<Product> findByColor(Integer productCategoryId, String color, Pageable pageable) {
        return dao.findByColor(productCategoryId, color, pageable);
    }

    @Override
    public Page<Product> findByCaId(Integer productCategoryId, int categoryId, Pageable pageable) {
